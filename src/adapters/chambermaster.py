...
class ChamberMasterAdapter(BaseDirectoryAdapter):
    ...
    def _extract_category_links(self, html: str) -> list[str]:
        ...
        Supports:
        - Static category links such as /list/ql/ and /list/category/
        - Category dropdown options using data-slug-id
        - Search and alphabetical directory links
        """
        soup = BeautifulSoup(html or "", "html.parser")
        categories: set[str] = set()

        # 1. Extract normal anchor-based category links.
        for anchor in soup.select("a[href]"):
            ...
            if (
                "/list/category/" in href_lower
                or "/list/ql/" in href_lower
            ):
                categories.add(
                ...
                "/list/search" in href_lower
                or "/list/searchalpha/" in href_lower
            ):
                categories.add(
                    urljoin(self.source_url, href)
                )

        # 2. Extract categories stored in the ChamberMaster dropdown.
        for option in soup.select(
            "#category-select option[data-slug-id]"
        ):
            slug_id = (
                option.get("data-slug-id") or ""
            ).strip()
        ...
        self._debug(
            "DEBUG ChamberMaster categories:",
            len(result),
        )
    ...
    def _filter_categories_by_request(
        self,
        categories: list[str],
    ) -> list[str]:
        """
        Filter ChamberMaster category URLs using requested
        services and keyword.
        """
        ...
        if keyword:
            requested_terms.append(keyword)

        requested_terms = list(
            dict.fromkeys(requested_terms)
        )

        self._debug(
            "DEBUG ChamberMaster requested category terms:",
            requested_terms,
        )

        if not requested_terms:
            self._debug(
                "DEBUG ChamberMaster no requested terms; "
        ...
        for category_url in categories:
            searchable = str(category_url).lower()

            if any(
                term in searchable
                for term in requested_terms
            ):
                matched_categories.append(category_url)

        self._debug(
            "DEBUG ChamberMaster matching categories:",
            matched_categories,
        )
    ...
    async def _discover_member_urls_from_category(
        ...
        self._debug("DEBUG ENTER CATEGORY:", current_url)
        visited_pages = set()
        category_name = ""
        ...
        while current_url:
            ...
            if len(visited_pages) > 1:
                self._debug(
                    "DEBUG ChamberMaster test page limit reached:",
                    len(visited_pages) - 1,
                )
                break
            ...
            self._debug("DEBUG AFTER GOTO:", page.url)

            await page.wait_for_timeout(2000)

            html = await page.content()
            self._debug("DEBUG HTML LENGTH:", len(html))
    ...
    async def discover_member_urls(
        self,
        page,
        ...
        self._debug("=" * 70)
        self._debug("ENTER discover_member_urls()")
        self._debug("CATEGORY URL COUNT:", len(category_urls))
        self._debug("CATEGORY URLS:", category_urls)
        self._debug("=" * 70)
        ...
        max_listings = max(
            1,
            int(self.discovery.max_results),
        )

        self._debug(
            "DEBUG ChamberMaster effective maxListings:",
            max_listings,
        )

        for i, category_url in enumerate(
            category_urls,
            start=1,
        ):
            try:
                self._debug(
                    f"DEBUG ChamberMaster category "
                    f"{i}/{len(category_urls)}:",
                    category_url,
                )
                ...
                if len(member_map) >= max_listings:
                    self._debug(
                        "DEBUG ChamberMaster maxListings reached; "
                        "stopping category discovery:",
                        len(member_map),
                        "limit:",
                        max_listings,
                    )
    ...
    async def crawl(self, page, max_records: int = 5) -> list[dict[str, Any]]:
        ...
        self.stats["categories_discovered"] = len(
            categories
        )

        self._debug(
            "DEBUG ChamberMaster categories found:",
            len(categories),
        )
        ...
        if not matched_categories:
            self._debug(
                "DEBUG ChamberMaster no matching categories "
        ...
        member_urls = await self.discover_member_urls(
            page,
            matched_categories,
        )
